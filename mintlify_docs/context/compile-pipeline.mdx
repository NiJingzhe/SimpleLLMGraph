...
description: How mutations become the final LLM request through the two-stage compile boundary
---

# Compile Pipeline

The compile pipeline transforms raw context state + pending mutations into the final messages sent to the LLM provider. It's a two-stage process with a clean boundary between state reduction and request rendering.
...
│       • Apply all pending mutations to the transcript
...
`reduce_turn_context` takes the current transcript plus all pending mutations and produces a clean, mutation-applied transcript.
...
1. **Apply mutations** — `apply_mutations(transcript, pending_mutations)` processes each mutation in order:
...
`convert_to_llm_request` takes the reduced context and produces the final messages for the provider.
...
Every iteration goes through the full pipeline. No shortcuts, no "just append to the list." This guarantees that even after 50 tool calls, the context state is consistent and auditable.
...
For most users, this is invisible. You write a function, it works.

But when you need to:
- **Debug context issues** → Check what mutations were produced and in what order
- **Inject custom mutations** → Produce typed `ContextMutation` objects from hooks
- **Understand system prompt behavior** → Know the priority order in Stage 2
- **Build custom state management** → Use `reduce_turn_context` directly

...the pipeline gives you complete visibility and control.
