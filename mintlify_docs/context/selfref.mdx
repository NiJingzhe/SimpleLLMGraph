...
- `collect_context_mutations()` — provides selfref-originated mutations before each compile
...
@llm_chat(self_reference_key="agent_main")
        ...
        │    → may emit Experience/Summary mutations     │
        │                                                │
        │  After turn ends:                              │
        │    → finalize()                                │
...
| `runtime.selfref.context.remember(text)` | Stores a durable experience → emits `ExperienceRememberMutation` |
| `runtime.selfref.context.forget(experience_id)` | Removes an experience → emits `ExperienceForgetMutation` |
| `runtime.selfref.context.compact(goal, instruction, discoveries, completed, current_status, likely_next_work, relevant_files_directories, remember=[])` | Queues context compaction → eventually emits `ContextSummaryMutation` |
...
- `remember("...")` → adds an experience immediately (via mutation)
...
2. After the current tool batch completes, `ContextSummaryMutation` is applied
