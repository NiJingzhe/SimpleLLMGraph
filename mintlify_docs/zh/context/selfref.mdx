...
- `collect_context_mutations()` ——在每次编译前提供来自 selfref 的 mutation
...
@llm_chat(self_reference_key="agent_main")
        ...
        │    → 可能产出 Experience/Summary mutation        │
        │                                                │
        │  轮次结束后：                                    │
        │    → finalize()                                │
        │    → 将更新后的历史持久化到后端                    │
...
| `runtime.selfref.context.remember(text)` | 存储持久经验 → 产出 `ExperienceRememberMutation` |
| `runtime.selfref.context.forget(experience_id)` | 移除经验 → 产出 `ExperienceForgetMutation` |
| `runtime.selfref.context.compact(goal, instruction, discoveries, completed, current_status, likely_next_work, relevant_files_directories, remember=[])` | 将上下文压缩加入队列 → 最终产出 `ContextSummaryMutation` |
...
处理过程：
1. 压缩被**加入队列**（不会立即应用）
2. 当前工具批次完成后，`ContextSummaryMutation` 被应用
3. 系统提示词被保留
4. 工作对话记录被替换为摘要消息
