...
3. An `AssistantTruncatedMutation` is emitted with:
...
2. For each cancelled tool, a `ToolCancelledMutation` is emitted with:
...
Abort always produces a valid final state. The transcript is never left with dangling tool calls or incomplete messages. The mutations ensure structural consistency.
...
async for output in research("...", _abort_signal=signal):
    ...
```
